...
- **3D form lighting** (`form`): describe a head, an apple or a fold of cloth as a silhouette plus
  bumps and hollows. pdnart lights it with a soft terminator, occlusion in the hollows, reflected
  light and a highlight.
- **painterly repainting** (`painterly`): repaints a smooth study, or a reference photo, with real
  brush strokes. It works from big brushes to small ones, adds strokes only where detail is needed,
  and bends them around the form the way a painter does.

| Landscape: 3,200 strokes over 8 layers | Portrait: lit 3D forms repainted with about 23,000 brush strokes |
|---|---|
| ![landscape](examples/landscape.png) | ![portrait](examples/portrait.png) |

*Both scenes above are made entirely from pdnart ops, generated by `examples/landscape.py` and
`examples/portrait.py`. The landscape is painted stroke by stroke. The portrait follows a classical
process:*
1. *Hidden study layers model the head, hair, neck and dress as lit 3D forms, with bumps for the
   brow, cheekbones, nose, lips and chin, and hollows for the eye sockets.*
2. *`painterly` repaints the studies in oil strokes, and a translucent `copy` of the study glazes the
   skin.*
3. *Eyes, brows, lashes and hair strands are painted on top with small brushes.*

*The brush presets side by side:*
...
1. Model each mass as a lit `form` on a hidden study layer.
2. Repaint the studies with `painterly` strokes.
3. Glaze where the paint should look smooth.
4. Paint crisp details.
5. Finish with linework.
...
`painterly` also works from a reference photo: `{"op": "painterly", "source": {"path": "photo.jpg"},
"sizes": [32, 16, 8, 4]}`. Relative paths are resolved next to the scene file.
...
| `form` | region (silhouette) | bumps `[{x,y,rx,ry,h,angle,profile}]`, inflate, light `[x,y,z]`, ramp, wrap, occlusion, ambient, bounce, shine |
| `painterly` | source (layer name, list of layers, or `{path,x,y,w,h}`) | sizes, threshold, length `[min,max]`, curvature, angle, color_jitter, region, brush + brush settings |
| `copy` | source (as for `painterly`) | opacity, clip, lock_alpha (for example, a translucent glaze of a study) |
